...
import { Plus, Search, RefreshCw, Pencil, Trash2, GitBranch } from 'lucide-react';
...
const EMPTY = { kode: '', nama: '', is_active: 1 };

export default function DivisionsPage() {
  const { data, pagination, loading, error, filters, create, update, remove, setPage, setSearch, refetch } = useTable(divisionService);
  ...
  const openEdit   = (row) => { setForm({ kode: row.kode, nama: row.nama, is_active: row.is_active }); setEditTarget(row); setFormOpen(true); };
    ...
    <div className="space-y-3 animate-fade-in">
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-ink-muted mt-0.5">Kelola data divisi / instalasi dalam sistem</p>
        </div>
      ...
      <div className="flex items-center gap-3 p-4 rounded-xl border" style={{ background: 'var(--c-card)', borderColor: 'var(--c-border)' }}>
        <div className="relative flex-1 max-w-sm">
          ...
          <input type="text" placeholder="Cari kode atau nama divisi..." value={filters.search} onChange={(e) => setSearch(e.target.value)} className="input-field pl-9 w-full" />
        </div>
      </div>
        ...
        <table className="w-full" style={{ minWidth: 520 }}>
          <thead>
            <tr style={{ background: 'var(--c-hover)', borderBottom: '1px solid var(--c-border)' }}>
              ...
              <th className="table-th">Nama Divisi</th>
          ...
          <tbody>
            {loading && (
              <tr><td colSpan={5} className="table-td text-center text-ink-faint py-10">Memuat data...</td></tr>
            )}
            {!loading && data.length === 0 && (
              <tr><td colSpan={5} className="table-td text-center text-ink-faint py-10">Belum ada data</td></tr>
            )}
            {!loading && data.map((row, i) => (
              <tr key={row.id} className="table-row">
                <td className="table-td text-ink-faint text-xs">{(filters.page - 1) * filters.limit + i + 1}</td>
                ...
                <td className="table-td text-right">
                  <div className="flex items-center justify-end gap-1">
                    <button onClick={() => openEdit(row)} className="w-7 h-7 inline-flex items-center justify-center rounded-md text-ink-faint hover:bg-[var(--c-hover)] transition-all cursor-pointer">
                      <Pencil size={13} />
                    </button>
                    <button onClick={() => setDeleteTarget(row)} className="w-7 h-7 inline-flex items-center justify-center rounded-md text-red-400 hover:bg-red-50 transition-all cursor-pointer">
                      <Trash2 size={13} />
                    </button>
                  </div>
                </td>
      ...
      <Modal open={formOpen} onClose={() => setFormOpen(false)} title={editTarget ? 'Edit Divisi' : 'Tambah Divisi'} size="sm">
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input {...field('kode')} required placeholder="DIV-IGD" className="input-field w-full" style={{ textTransform: 'uppercase' }} />
            </div>
            <div>
          ...
          <div>
            <label className="text-xs font-semibold text-ink-faint uppercase tracking-wide block mb-1.5">Nama Divisi <span className="text-red-500">*</span></label>
            <input {...field('nama')} required placeholder="Instalasi Gawat Darurat" className="input-field w-full" />
          </div>
      ...
      <Modal open={!!deleteTarget} onClose={() => setDeleteTarget(null)} title="Hapus Divisi" size="sm">
        <div className="space-y-4">
          <p className="text-sm text-ink-muted">Yakin ingin menghapus divisi <span className="font-semibold text-ink">{deleteTarget?.nama}</span>?</p>
