...
const getAll = async (req, res) => {
  try {
    ...
    if (search) {
      where += ' AND (kode LIKE ? OR nama LIKE ? OR direktur LIKE ?)';
      params.push(`%${search}%`, `%${search}%`, `%${search}%`);
    }

    if (all === '1') {
      const [rows] = await pool.query(`SELECT * FROM hospitals ${where} ORDER BY nama`, params);
      return res.json({ data: rows });
    }

    const offset = (parseInt(page) - 1) * parseInt(limit);
    const [[{ total }]] = await pool.query(`SELECT COUNT(*) as total FROM hospitals ${where}`, params);
    const [rows] = await pool.query(
      `SELECT * FROM hospitals ${where} ORDER BY nama LIMIT ? OFFSET ?`,
...
const getOne = async (req, res) => {
  try {
    const [[row]] = await pool.query('SELECT * FROM hospitals WHERE id = ? AND deleted_at IS NULL', [req.params.id]);
    if (!row) return res.status(404).json({ message: 'Rumah sakit tidak ditemukan' });
    res.json(row);
  } catch (err) {
    res.status(500).json({ message: err.message });
  }
...
const remove = async (req, res) => {
  try {
    const [[check]] = await pool.query('SELECT id FROM hospitals WHERE id = ? AND deleted_at IS NULL', [req.params.id]);
    if (!check) return res.status(404).json({ message: 'Rumah sakit tidak ditemukan' });
    await pool.query('UPDATE hospitals SET deleted_at = NOW(), updated_by = ? WHERE id = ?', [req.user.id, req.params.id]);
